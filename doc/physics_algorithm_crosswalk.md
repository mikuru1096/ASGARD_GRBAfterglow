...
| photon field | `Seed_syn(nu,R)` | `n_nu = u_nu/(h nu)` | `syn_seed_chi`, Python `photon_density_hz_to_gev` direct unit conversion | Observer luminosity 不能直接当 target density，必须经 shell volume/escape time/单位转换。 |
