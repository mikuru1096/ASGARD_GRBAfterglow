...
- Latest A&A abstract compatibility backup: `/home/wangyun/Desktop/HEtools_web_beta_v03_backups/gpt_aa_abstract_fix_20260628_112433`
- Latest caption split fix backup: `/home/wangyun/Desktop/HEtools_web_beta_v03_backups/gpt_caption_split_fix_20260628_172346`
...
- 手工修复 arXiv 缓存中的 TeX 后，不能只跑一遍 `xelatex` 就发布 PDF。`astro-ph/0112087` 的交叉引用和引用项需要多遍编译收敛；发布前必须检查最终 `merge.log` 和 `merge_translate_zh.log` 中没有 `Citation ... undefined`、`Reference ... undefined`、`undefined references` 或 `Rerun to get cross-references right`。远端 `translation/translate_zh.pdf` 已于 2026-06-27 用四遍 `xelatex` 更新，最终日志的 reference scan 为空。
- `0810.2961` 暴露了 A&A `aa.cls` 摘要命令边界：A&A 使用 `\abstract{context}{aims}{methods}{results}{conclusions}` 五参数命令，且源码常写成 `\abstract` 后换行再给多个 `{...}` 参数；`aa.cls` 不提供 `abstract` 环境。`crazy_functions/latex_fns/latex_toolbox.py` 现在用 TeX brace parser 识别 `abstract` 环境、普通 `\abstract{...}` 和 A&A 多参数 `\abstract`，避免误判缺摘要后插入 `\begin{abstract}...\end{abstract}`；`crazy_functions/latex_fns/latex_actions.py` 的翻译警告也通过同一入口插入到摘要第一个参数内部。远端已重新生成 `0810.2961/workfolder/merge.pdf`、`merge_translate_zh.pdf` 和 `translation/translate_zh.pdf`；缓存修复备份位于 `/home/wangyun/Desktop/gpt_academic_web/gpt_log/arxiv_cache/0810.2961/repair_backup_20260628_112605`，最终 `merge.log` 和 `merge_translate_zh.log` 的 error/reference scan 均为空。
- `0810.2961` 还暴露了图注切分边界：大量 A&A 图注写成 `\caption[]{...}` 或 `\caption[] {...}`，旧规则只反向开放 `\caption{...}`，导致图注正文被 figure/table 保护区吞掉而保持英文；短表注也会被 `post_process` 的 42 字符阈值再次保护。`crazy_functions/latex_fns/latex_actions.py` 现在支持带可选参数和空格的 caption 主体；`crazy_functions/latex_fns/latex_toolbox.py::post_process` 保留短文本保护，但当短文本位于 `\caption...{}` 主体中时不再保护。远端 dry-run 已确认 0810.2961 的 11 个 caption 均进入待翻译段；现有缓存的 `merge_translate_zh.tex` 已手工替换为中文 caption 并重新四遍 `xelatex` 编译，发布到 `translation/translate_zh.pdf`。缓存修复备份位于 `/home/wangyun/Desktop/gpt_academic_web/gpt_log/arxiv_cache/0810.2961/caption_repair_backup_20260628_172550`，最终 `merge.log` 和 `merge_translate_zh.log` 的 error/reference scan 均为空。
