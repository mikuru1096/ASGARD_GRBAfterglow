...
本文固定当前 public API 已暴露但 backend 不支持或只部分支持的边界。原则：不制造伪支持，不添加兜底路径；未支持边界只有在明确物理问题需要时才进入实现。

## 明确不支持

- Jet spreading：
  - Public jet constructors 接受 `spreading` 参数。
...
- User-defined `Medium` kernel dispatch：
  - `Medium.to_kernel_params()` 对 custom medium 直接抛出 `NotImplementedError`。
...
- `fullhide_1d` 之外的 thermal electrons：
  - `solve_electron(...)` 在 `thermal_electrons=True` 且 solver 不是 `fullhide_1d` 时直接抛出 `NotImplementedError`。
...
- 2D electron solvers：
  ...
  - Hadronic main path 仍要求 1D electron solver；2D/chi hadronic transport 已在 `doc/hadronic_chi_transport_decision.md` 中明确暂不实现。
- Reverse-shock hadronic：
  - light path 是 RS proton transport + proton synch。
  - full-chain path 复用 formal 1D hadronic kernels；不是 2D/chi RS hadronic transport。
- Pair cascade：
  - `pair_cascade_iterations > 1` 是 shell-sequence gamma-gamma pair/synch cascade。
