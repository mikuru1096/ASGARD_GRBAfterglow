# 2D / Chi-Resolved Hadronic Transport Decision
...
- `PhotonFieldState` 仍是 shell-level 契约：`seed_frequency_hz` 加 `forward_syn_seed`、`hadronic_forward_ssc_seed`、`hadronic_target_seed`、`absorption_*_seed`。这些 seed arrays 是 `frequency x radius`，没有 χ 轴。
