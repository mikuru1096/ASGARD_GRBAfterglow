...
如果只改变 electron cooling 而没有同步改变 IC photon source，电子能量会从系统中消失。当前 joint 路径通过同一 seed 传入 coupled electron pass，并用 smoke test 检查 IC loss 与 IC photon source 的壳层积分闭合。
