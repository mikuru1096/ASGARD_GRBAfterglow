...
def evolve_hummer2010_transport_1d(
    ...
    *,
    ...
    """Advance coupled proton/neutron transport with Hummer2010 loss and reinjection."""
    r_tobs_arr = np.asarray(r_tobs_s, dtype=float)
    r_gamma_arr = np.asarray(r_gamma, dtype=float)
    radius_arr = np.asarray(radius_cm, dtype=float)
    shell_energy_arr = np.asarray(shell_energy_inj_erg, dtype=float)
    b_field_arr = np.asarray(b_field_g, dtype=float)
    v_seed_arr = np.asarray(v_seed_hz, dtype=float)
    seed_target_arr = np.asarray(seed_target_hz, dtype=float)

    if r_tobs_arr.ndim != 1:
        raise ValueError("r_tobs_s must be a 1d array.")
    if r_gamma_arr.ndim != 1:
        raise ValueError("r_gamma must be a 1d array.")
    if radius_arr.ndim != 1:
        raise ValueError("radius_cm must be a 1d array.")
    if shell_energy_arr.ndim != 1:
        raise ValueError("shell_energy_inj_erg must be a 1d array.")
    if b_field_arr.ndim != 1:
        raise ValueError("b_field_g must be a 1d array.")
    if not (r_tobs_arr.shape == r_gamma_arr.shape == radius_arr.shape == shell_energy_arr.shape == b_field_arr.shape):
        raise ValueError("r_tobs_s, r_gamma, radius_cm, shell_energy_inj_erg and b_field_g must share shape.")
    ...
    if seed_target_arr.shape != (v_seed_arr.size, radius_arr.size):
        raise ValueError("seed_target_hz must have shape (len(v_seed_hz), len(radius_cm)).")
    if int(num_gam_p) < 2:
        raise ValueError("num_gam_p must be >= 2.")
    if int(num_nu_nu) < 2:
        raise ValueError("num_nu_nu must be >= 2.")

    num_r = radius_arr.size
    interaction_active = bool(include_pg) or bool(include_neutrino)
    shell_volume_arr = _shell_volumes_from_radius(radius_arr)

    gam_p = _build_hadronic_gamma_grid(
        int(num_gam_p),
        radius_arr,
        r_gamma_arr,
        b_field_arr,
    ...
    d_n_gam_p = np.zeros((gam_p.size, num_r), dtype=float)
    d_n_gam_n = np.zeros((gam_p.size, num_r), dtype=float)
    l_had_pg_gamma = np.zeros((v_seed_arr.size, num_r), dtype=float)
    neutrino_luminosity = np.zeros((int(num_nu_nu), num_r), dtype=float)
    am3_process_power = np.zeros((len(AM3_PROCESS_LABELS), gam_p.size, num_r), dtype=float)
    process_luminosity = np.zeros((len(AM3_PROCESS_LABELS), photon_energy_gev.size, num_r), dtype=float)
    proton_reinj_rate = np.zeros((gam_p.size, num_r), dtype=float)
    neutron_reinj_rate = np.zeros((gam_p.size, num_r), dtype=float)
    proton_loss_rate = np.zeros((gam_p.size, num_r), dtype=float)
    neutron_loss_rate = np.zeros((gam_p.size, num_r), dtype=float)
    photon_loss_rate = np.zeros((v_seed_arr.size, num_r), dtype=float)

    d_n_prev_p = np.zeros(gam_p.size, dtype=float)
    d_n_prev_n = np.zeros(gam_p.size, dtype=float)
    zero_source = np.zeros(gam_p.size, dtype=float)

    for i_r in range(num_r):
        dt_s = _hadronic_shell_dt(r_tobs_arr, i_r)
        t_dyn_s = _hadronic_dynamical_time(radius_arr[i_r], r_gamma_arr[i_r])
        gam_p_min = max(1.1, float(r_gamma_arr[i_r]))

        q_inj = _proton_injection_powerlaw(
            gam_p,
            float(p_p),
            max(float(shell_energy_arr[i_r]), 0.0),
            gam_p_min,
            float(gam_p[-1]),
        )
        loss_p = _proton_continuous_loss_rate(
            gam_p,
            max(float(b_field_arr[i_r]), 1.0e-30),
            t_dyn_s,
        )
        loss_n = gam_p / max(t_dyn_s, 1.0)
        d_n_cont_p = _advance_energy_loggamma(gam_p, d_n_prev_p, q_inj, loss_p, dt_s)
        d_n_cont_n = _advance_energy_loggamma(gam_p, d_n_prev_n, zero_source, loss_n, dt_s)
        ...
        if interaction_active:
            ...
            proton_density_per_gev = d_n_cont_p / (shell_volume_cm3 * PROTON_MASS_GEV)
            neutron_density_per_gev = d_n_cont_n / (shell_volume_cm3 * PROTON_MASS_GEV)
            ...
            backend = solve_hummer2010_pgamma(
                ...
                neutron_density_per_gev=neutron_density_per_gev,
            )
            ...
            # dN/(dE dt dV) -> dN/(dgamma dt) for reinjection source terms.
            q_reinj_p = backend.proton_reinjection_rate_per_gev * shell_volume_cm3 * PROTON_MASS_GEV
            q_reinj_n = backend.neutron_reinjection_rate_per_gev * shell_volume_cm3 * PROTON_MASS_GEV
            d_n_next_p = d_n_cont_p + dt_s * (q_reinj_p - backend.proton_loss_rate * d_n_cont_p)
            d_n_next_n = d_n_cont_n + dt_s * (q_reinj_n - backend.neutron_loss_rate * d_n_cont_n)
            d_n_next_p = np.maximum(d_n_next_p, 0.0)
            d_n_next_n = np.maximum(d_n_next_n, 0.0)
            ...
            proton_energy_weight = d_n_next_p * proton_energy_gev
            ...
            photopion_total = float(trapezoid(process_luminosity[0, :, i_r], photon_energy_gev))
            ...
            d_n_next_p = d_n_cont_p
            d_n_next_n = d_n_cont_n

        d_n_gam_p[:, i_r] = d_n_next_p
        d_n_gam_n[:, i_r] = d_n_next_n
        d_n_prev_p = d_n_next_p
        d_n_prev_n = d_n_next_n
    ...
    return HadronicTransport1DOutput(
        gam_p=gam_p,
        proton_distribution=d_n_gam_p,
        neutron_distribution=d_n_gam_n,
...
def _shell_volumes_from_radius(radius_cm: np.ndarray) -> np.ndarray:
    ...
    return (4.0 / 3.0) * math.pi * np.maximum(radius**3 - prev_radius**3, 0.0)
