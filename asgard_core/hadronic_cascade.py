...
def compute_iterative_pair_cascade(
    ...
    if e_ph.ndim != 1 or e_e.ndim != 1 or n_ph_primary.shape != e_ph.shape:
        raise ValueError("pair cascade requires 1D photon/electron grids and matching photon density.")
    if not (np.all(np.isfinite(e_ph)) and np.all(np.isfinite(e_e)) and np.all(np.isfinite(n_ph_primary))):
        raise ValueError("pair cascade inputs must be finite.")
    if np.any(e_ph <= 0.0):
        raise ValueError("pair cascade photon energies must be positive.")
    if np.any(e_e <= 0.0):
        raise ValueError("pair cascade electron energies must be positive.")
    if np.any(np.diff(e_ph) <= 0.0) or np.any(np.diff(e_e) <= 0.0):
        raise ValueError("pair cascade energy grids must be strictly increasing.")
    if np.any(n_ph_primary < 0.0):
        raise ValueError("pair cascade photon density must be non-negative.")
    if b_field_g < 0.0:
        raise ValueError("pair cascade magnetic field must be non-negative.")
    if max_iterations < 1:
        raise ValueError("pair cascade max_iterations must be positive.")
    ...
    for iteration in range(max_iterations):
        cascade_syn, photon_loss_iter, absorbed = hadronic_fortran_module.fs_hadronic_pair_cascade_step(
            e_ph, n_ph, e_e,
            float(b_field_g),
            path_time,
        )
        cascade_syn = np.asarray(cascade_syn, dtype=float)
        if np.any(cascade_syn < 0.0):
            raise RuntimeError("pair cascade Fortran kernel returned negative synchrotron emissivity.")
        photon_loss = np.asarray(photon_loss_iter, dtype=float)
        if np.any(photon_loss < 0.0):
            raise RuntimeError("pair cascade Fortran kernel returned negative photon loss rate.")
        total_absorbed += float(absorbed)

        cascade_new = cascade_syn / (e_ph * constants.para_h_gev)
        old_norm = float(np.max(cascade_accum))
        new_norm = float(np.max(cascade_new))
        rel_change = 0.0
        if iteration > 0 and old_norm > 0.0:
            rel_change = float(np.max(np.abs(cascade_new - cascade_accum)) / old_norm)
        cascade_accum = cascade_new

        if iteration > 0 and (
            (old_norm == 0.0 and new_norm == 0.0)
            or (old_norm > 0.0 and rel_change < convergence_rtol)
        ):
            converged = True
    ...
    tau_pair = photon_loss * path_time
