...
def main() -> None:
    times_s = np.logspace(2.0, 6.0, 96)
    frequencies_hz = np.array([9.0e9, 4.84e14, 2.4e17], dtype=float)

    model = _build_model()
    with patch.object(asgard_api, "solve_model_state_from_setup", wraps=asgard_api.solve_model_state_from_setup) as solve_mock:
        adaptive = model.flux_density_grid(times_s, frequencies_hz)
        solve_count = solve_mock.call_count

    reference_model = _build_model()
    reference = reference_model._compute_raw(times_s, frequencies_hz)
    ...
    print("PASS: adaptive flux grid check succeeded.")
