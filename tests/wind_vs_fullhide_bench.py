...
from ASGARD import Fitter, Model, Observer, ParamDef, Radiation, Scale, Setups, TophatJet, Wind, units, make_empty_obs, make_flux_density_entry
...
def _cases_for_solver(solver: str) -> list[dict]:
    ...
    for idx, (name, fn) in enumerate(cases, start=1):
        results.append(_run_case(f"[{idx}/{total}] {solver}:{name}", lambda fn=fn: fn()))
    return results
