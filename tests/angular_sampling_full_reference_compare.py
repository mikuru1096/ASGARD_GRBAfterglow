...
def _python_surface_axisym_reference(
    ...
    *,
    ...
    from ASGARD.api_model import (
        ...
        _observe_surface_element_parts,
        _solve_patch_state,
    )
    ...
    theta_edges = np.linspace(0.0, model.jet.theta_max, patch_theta + 1)
    theta_centers = 0.5 * (theta_edges[:-1] + theta_edges[1:])
    phi_centers = np.linspace(0.0, np.pi, patch_phi)
    phi_weights = np.full(patch_phi, np.pi / float(patch_phi - 1))
    phi_weights[0] *= 0.5
    phi_weights[-1] *= 0.5
    phi_weights *= 2.0
    total = np.zeros((freqs.size, times.size), dtype=float)
    for i_theta, theta_center in enumerate(theta_centers):
        ...
        if e_iso <= 0.0 or gamma0 <= 1.0:
            continue
        dcos = np.cos(theta_edges[i_theta]) - np.cos(theta_edges[i_theta + 1])
        config = _build_fit_config_for_patch(
            model,
            phi_center=0.0,
            theta_v=0.0,
            opening_angle_jet=float(np.sqrt(dcos * phi_weights[0] / np.pi)),
            e_iso=e_iso,
            gamma0=gamma0,
        ...
        for i_phi, phi_center in enumerate(phi_centers):
            theta_v = _angular_separation(float(theta_center), float(phi_center), theta_obs, model.observer.phi_obs)
            boundary[9] = theta_v
            observed = _observe_surface_element_parts(state, times, freqs, float(dcos * phi_weights[i_phi]))
...
def main() -> None:
    ...
    for i_ratio, ratio in enumerate(ratios):
        ...
        candidate_flux[i_ratio] = np.asarray(model.flux_density_grid(times, freqs).total, dtype=float)
        patches = model.details().patches
        candidate_patch_counts.append(len(patches))
        candidate_theta_counts.append(len({round(float(patch["theta"]), 14) for patch in patches}))
        candidate_phi_counts.append(len({round(float(patch["phi"]), 14) for patch in patches}))
