...
BASIC_FREQS = np.logspace(5.0, 22.0, 160)
...
def _build_basic_lc_spec() -> Path:
    ...
    freqs = BASIC_FREQS
    epochs = BASIC_EPOCHS
    sed_as = model_asgard.flux_density_grid(epochs, freqs).total
    sed_vg = model_vegas.flux_density_grid(epochs, freqs).total
    ...
    axes[1].set_ylabel(r"Flux Density (erg/cm$^2$/s/Hz)")
...
def _build_spectrum_compare() -> Path:
    model_asgard = _build_asgard_model(include_ssc=True)
    model_vegas = _build_vegas_model(include_ssc=True)
    times = SPEED_SPEC_TIMES
    freqs = np.logspace(8.0, 27.0, 160)

    as_res = model_asgard.flux_density_grid(times, freqs)
    vg_res = model_vegas.flux_density_grid(times, freqs)

    fig, axes = plt.subplots(1, 3, figsize=(12.6, 4.0), dpi=200, sharey=True)
    for i, t_obs in enumerate(times):
        ax = axes[i]
        ax.loglog(freqs, np.asarray(as_res.total[:, i], dtype=float), color="C0", lw=1.8, label="ASGARD")
        ax.loglog(freqs, np.asarray(vg_res.total[:, i], dtype=float), color="C1", ls="--", lw=1.5, label="Vegas")
        ax.set_title(_label(t_obs, "s"))
        ax.set_xlabel("Frequency [Hz]")
        ax.grid(alpha=0.25, which="both")
    axes[0].set_ylabel(r"$F_\nu$ [erg s$^{-1}$ cm$^{-2}$ Hz$^{-1}$]")
    axes[0].legend(fontsize=8, loc="best")
    fig.suptitle("Spectrum comparison", y=1.02)
...
def main() -> None:
    builders = [
        ...
        ("spectrum_compare", _build_spectrum_compare),
