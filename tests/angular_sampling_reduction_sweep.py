...
def main() -> None:
    ...
    for ratio in ratios:
        ...
        flux, _grid = patch_flux_grid(
            model, times, freqs, sampling="uniform", patch_theta=reference_theta, patch_phi=reference_phi
        )
        reference_flux.append(np.asarray(flux, dtype=float))
    ...
    for patch_theta, patch_phi in candidate_grids:
        ...
        for ratio in ratios:
            ...
            candidate_flux.append(np.asarray(flux, dtype=float))
