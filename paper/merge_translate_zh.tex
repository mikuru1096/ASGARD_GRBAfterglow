...
\subsubsection{角向对产生核}


\gptareaderanchor{gpta-seg-000222}%
对产生的截面依赖于碰撞角度。壳核以中点角度对局部光子方向进行平均，
\begin{equation}
\begin{aligned}
...
物理阈值是 \(s_{aj}>1\)。满足 \(s_{aj}\ge s_{\max}\) 的单元位于有限列表评估窗口之外；\(s_{\max}\) 并非物理阈值。横截面为
\begin{equation}
\begin{aligned}
\beta_s&=(1-s^{-1})^{1/2},\\
...
这里 \(\sigma_{\rm es}\) 是电子散射截面。分支 \(\sigma_{\gamma\gamma}=0\) 对 \(s\le1\) 是物理对产生的阈值。

\subsubsection{壳层光深组装}


\gptareaderanchor{gpta-seg-000225}%
壳层光学深度将目标光子在频率和角度上求和。对于半径\(R_i\)、洛伦兹因子\(\Gamma_i\)、且\(\beta_i=(1-\Gamma_i^{-2})^{1/2}\)的壳层，定义
\begin{equation}
\begin{aligned}
...
此处 \(\Delta'_{\gamma\gamma,i}\) 是共动壳层深度，\(\eta_{\gamma\gamma}\) 是其固定几何因子，\(W_{ia}\) 是角权重，\(n_j\) 是单元 \(j\) 中的靶光子密度。因子 \(1/2\) 是对 \(-1\le\mu\le1\) 进行平均，\(\tau_{\rm ex}\) 添加由其他局部模块提供的光学深度。返回的有限单元存活因子是
\begin{equation}
\begin{aligned}
