...
subroutine hadronic_pair_production_operator(num_ph,photon_energy_gev,photon_density_per_gev,num_e,electron_energy_gev, &
    ...
    gm_e = electron_energy_gev/electron_mass_gev
    ...
    absorbed_power_gev_per_cm3_s = sum(x_ph(ind_min_energy_pho+1:num_ph)*afpair_ssc(ind_min_energy_pho+1:num_ph)* &
                                       photons_log_ssc(ind_min_energy_pho+1:num_ph))*rate_ssc_cgs*electron_mass_gev
    injected_power_gev_per_cm3_s = sum(two*gm_e*epspair_ssc)*rate_ssc_cgs*electron_mass_gev
...
real(8) function hadronic_uniform_log_spacing(num_grid,grid)
    ...
    real(8) :: dln0
    if (num_grid < 2) then
        error stop "pair production grid must contain at least two points."
    end if
    dln0 = dlog(grid(2)/grid(1))
    do i=2,num_grid-1
        if (dabs(dlog(grid(i+1)/grid(i)) - dln0) > dmax1(1.0d-12,1.0d-6*dabs(dln0))) then
            error stop "pair production grid must be logarithmically uniform."
        end if
    end do
    hadronic_uniform_log_spacing = dln0
