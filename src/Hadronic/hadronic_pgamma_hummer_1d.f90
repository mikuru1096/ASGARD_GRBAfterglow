...
subroutine hadronic_pgamma_hummer_shell(num_gam_p,num_ph,num_nu,dt_s,radius_cm,gamma_bulk,b_field_g,shell_volume_cm3, &
    ...
    real(8) :: neutron_loss_interp(num_gam_p),zero_source(num_gam_p),divergence_rate
    ...
    do i=1,num_gam_p
        rate_dt=ploss(i)*dt_s
        if (ploss(i) > zero) then
            dn_next(i)=dn_trial(i)*exp(-rate_dt)+(one-exp(-rate_dt))/ploss(i)*shell_volume_cm3*preinj(i)*hadronic_proton_mass_gev
        else
            dn_next(i)=dn_trial(i)+dt_s*shell_volume_cm3*preinj(i)*hadronic_proton_mass_gev
        end if
    end do
