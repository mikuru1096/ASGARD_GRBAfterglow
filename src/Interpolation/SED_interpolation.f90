...
subroutine sed_interpolation_chi_surface_element(Boundary,R_Tobs1,R_front,F_chi,Tau_chi,R_chi,Gamma_chi,Chi_weight, &
    ...
    real(8), allocatable :: V_obs_log(:),V_seed_log(:)
    real(8) :: R_Tobs_chi(Num_R),log_domega_4pi
    real(8) :: log_gamma_lo,log_gamma_hi,segment_lo,segment_hi
    integer :: last_k2,k_start,lower_bound_real8
    logical :: monotonic_chi
        ...
        if (monotonic_chi) then
            ...
            do K1 = 1, Num_Tobs
                ...
                if (Tobs(K1) >= R_Tobs_chi(K2) .and. Tobs(K1) < R_Tobs_chi(K2+1)) then
                    ...
                    Ratio = (Tobs(K1)-R_Tobs_chi(K2))/(R_Tobs_chi(K2+1)-R_Tobs_chi(K2))
                    call project_surface_chi_segment_flux(I_chi,K2,K1,Ratio,DMu,log_domega_4pi, &
                                                          log_gamma_lo,log_gamma_hi)
                end if
            end do
        else
...
subroutine project_surface_chi_segment_flux(I_chi,K2,K1,Ratio,DMu,log_domega_4pi,log_gamma_lo,log_gamma_hi)
    implicit real(8)(A-H,O-Z)
    integer, intent(in) :: I_chi,K2,K1
    real(8), intent(in) :: Ratio,DMu,log_domega_4pi,log_gamma_lo,log_gamma_hi
    real(8) :: F_log_theta(Num_nu),V_seed_log_theta(Num_nu)
    real(8) :: log_doppler_redshift,doppler

    call compute_surface_chi_state(I_chi,K2,Ratio,DMu,log_gamma_lo,log_gamma_hi, &
                                   log_doppler_redshift,doppler,F_log_theta)
...
end subroutine project_surface_chi_segment_flux

subroutine compute_surface_chi_state(I_chi,K2,Ratio,DMu,log_gamma_lo,log_gamma_hi, &
                                     log_doppler_redshift,doppler,F_log_theta)
    implicit real(8)(A-H,O-Z)
    integer, intent(in) :: I_chi,K2
    real(8), intent(in) :: Ratio,DMu,log_gamma_lo,log_gamma_hi
    real(8), intent(out) :: log_doppler_redshift,doppler,F_log_theta(Num_nu)
    real(8) :: DG,Beta
    ...
    doppler = DG*(one-Beta*DMu)
    log_doppler_redshift = dlog(doppler)+dlog(one+z)
    call accumulate_surface_chi_cell_source(I_chi,K2,Ratio,F_log_theta)
end subroutine compute_surface_chi_state

subroutine accumulate_surface_chi_cell_source(I_chi,K2,Ratio,F_log_theta)
    implicit real(8)(A-H,O-Z)
    integer, intent(in) :: I_chi,K2
    real(8), intent(in) :: Ratio
    real(8), intent(out) :: F_log_theta(Num_nu)
    real(8) :: source_lo,source_hi,tau_front_lo,tau_front_hi,tau_cell_lo,tau_cell_hi,temp
    integer :: I_nu,J_chi
...
end subroutine accumulate_surface_chi_cell_source
