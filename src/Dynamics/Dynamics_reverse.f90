subroutine dynamics_reverse(Delta_t,e_r,b_r,p_r,f_e_r,Boundary,n,Num_R, &
                            T_cross,R_cross,e3_cross,gam20,R_Tobs,R_Gamma,R,M2,M3,B3)
    !$ use omp_lib
    use constants
    ...
    real(8), intent(in) :: Boundary(n),Delta_t,e_r,b_r,p_r,f_e_r
    real(8), intent(out) :: T_cross,R_cross,e3_cross,gam20,R_Tobs(Num_R),R(Num_R),M2(Num_R),M3(Num_R),B3(Num_R),R_Gamma(Num_R)
    ...
    para_m3=1d1; R_Gamma(1)=Eta_0-0.001d0; B3(1)=zero
    ...
    do I_tobs=1,Num_R
        ...
        call dynamics_rk4_reverse(F,dB3,T_cross,R_cross,e3_cross,gam20,T,H,Y,para_m_ej,Delta_0,eta_0,A_star, &
                                  dNe_ISM,Epsilon_b,Epsilon_e,p_f,f_e,e_r,b_r,p_r,f_e_r)
        R_Tobs(I_tobs)=T*(one+z); R_Gamma(I_tobs)=Y(1); R(I_tobs)=Y(2); M2(I_tobs)=Y(3); M3(I_tobs)=Y(4); B3(I_tobs)=dB3
    end do
...
subroutine F(dB3,T_cross,R_cross,e3_cross,gam20, &
    ...
    dgam2=dgam2*dR; D=[dgam2,dR,dm2,dm3]
end subroutine F
