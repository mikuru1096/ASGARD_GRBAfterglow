...
contains
    ...
    subroutine adaptive_resampling_log(x, f, n, m, g, indices, n_resampled, info)
        ! 输入参数
        ...
        real(dp) :: df_dlogx(n), df_abs(n)
        real(dp) :: dx_adj(n), df_weighted(n)
        real(dp) :: df_smooth(n), w(n), weighted_w(n), c(n)
        ...
        ! 在对数尺度上计算一阶导数
        df_dlogx(1) = (f(2) - f(1)) / dlogx(1)  ! 向前差分
        df_dlogx(n) = (f(n) - f(n-1)) / dlogx(n-1)  ! 向后差分
        ...
        ! 应用调整因子
        df_weighted = df_abs / dx_adj
        
        ! 平滑权重（简单移动平均）
        call moving_average(df_weighted, n, 5, df_smooth)
