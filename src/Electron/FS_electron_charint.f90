subroutine fs_electron_charint(Boundary,R_Tobs,R_Gamma,R,V_seed,n,Num_nu,Num_R,Num_gam_e,index_Y,index_syn_intger,n_threads, &
    ...
    real(8), allocatable :: q_left_source_shape(:),q_right_source_shape(:),prefix_source_shape(:)
    ...
    allocate(dEl_base(Num_gam_e),dEl_step(Num_gam_e),dN_x(Num_gam_e),dN_step(Num_gam_e), &
             ...
             q_left_source_shape(Num_gam_e),q_right_source_shape(Num_gam_e),prefix_source_shape(0:Num_gam_e))
        ...
        if (adaptive_substeps == 0) then
            if (is_uniform_density) then
                ...
                call electron_prepare_conservative_remap_nonuniform(Num_gam_e,x_edge,dF1_shape, &
                                                                    q_left_source_shape,q_right_source_shape,prefix_source_shape)
                ...
                do L=1,L1
                    ...
                    if (index_Y == 0) then
                        ...
                        call electron_characteristic_step_affine_u_prepared_source( &
                            Num_gam_e,dDR,x_edge,a_rad,b_ad,Q,dF1_shape, &
                            q_left_source_shape,q_right_source_shape,prefix_source_shape,dN_x,dN_step)
                    else
                        dEl_step=dEl_base
                        call electron_characteristic_step_piecewise_u_prepared_source( &
                            Num_gam_e,dDR,x_edge,gam_e,dEl_step,R_mid,Q,dF1_shape, &
                            q_left_source_shape,q_right_source_shape,prefix_source_shape,dN_x,dN_step)
                    end if
            ...
            do while (dR_left > zero)
                ...
                if (is_uniform_density) then
                    ...
                    if (index_Y == 0) then
                        b_ad=one/R_mid
                        call electron_characteristic_step_affine_u_prepared_source( &
                            Num_gam_e,dR_try,x_edge,a_rad,b_ad,Q,dF1_shape, &
                            q_left_source_shape,q_right_source_shape,prefix_source_shape,dN_x,dN_step)
                    else
                        dEl_step=dEl_base
                        call electron_characteristic_step_piecewise_u_prepared_source( &
                            Num_gam_e,dR_try,x_edge,gam_e,dEl_step,R_mid,Q,dF1_shape, &
                            q_left_source_shape,q_right_source_shape,prefix_source_shape,dN_x,dN_step)
                    end if
                else
