...
pure real(8) function electron_coord_from_xgamma(coord_kind, coord_scale, x_gamma) result(coord)
    ...
    case (electron_coord_log_four_velocity_sq)
        coord = electron_four_velocity_coord_from_xgamma(coord_scale, x_gamma)
    end select
...
pure real(8) function electron_xgamma_from_coord(coord_kind, coord_scale, coord) result(x_gamma)
    ...
    case (electron_coord_log_gamma)
        x_gamma = coord
    case (electron_coord_log_four_velocity_sq)
        x_gamma = electron_xgamma_from_four_velocity_coord(coord_scale, coord)
    end select
...
pure real(8) function electron_gamma_from_coord(coord_kind, coord_scale, coord) result(gamma)
    ...
    case (electron_coord_log_gamma)
        gamma = ten**coord
    case (electron_coord_log_four_velocity_sq)
        gamma = electron_gamma_from_four_velocity_coord(coord_scale, coord)
    end select
...
pure real(8) function electron_dxgamma_dcoord(coord_kind, coord_scale, coord) result(dxdy)
    ...
    case (electron_coord_log_gamma)
        dxdy = one
    case (electron_coord_log_four_velocity_sq)
        dxdy = electron_four_velocity_dxgamma_dcoord(coord_scale, coord)
    end select
end function electron_dxgamma_dcoord

pure real(8) function electron_four_velocity_coord_from_xgamma(coord_scale, x_gamma) result(coord)
    real(8), intent(in) :: coord_scale, x_gamma
...
end function electron_four_velocity_coord_from_xgamma

pure real(8) function electron_xgamma_from_four_velocity_coord(coord_scale, coord) result(x_gamma)
    real(8), intent(in) :: coord_scale, coord

    x_gamma = dlog10(electron_gamma_from_four_velocity_coord(coord_scale, coord))
end function electron_xgamma_from_four_velocity_coord

pure real(8) function electron_gamma_from_four_velocity_coord(coord_scale, coord) result(gamma)
    real(8), intent(in) :: coord_scale, coord

    gamma = dsqrt(one + coord_scale*(ten**coord - one))
end function electron_gamma_from_four_velocity_coord

pure real(8) function electron_four_velocity_dxgamma_dcoord(coord_scale, coord) result(dxdy)
    real(8), intent(in) :: coord_scale, coord
    real(8) :: gamma

    gamma = electron_gamma_from_four_velocity_coord(coord_scale, coord)
    dxdy = coord_scale*ten**coord/(two*gamma*gamma)
end function electron_four_velocity_dxgamma_dcoord
...
subroutine electron_build_four_velocity_grid(num_gamma, gamma_min, gamma_max, coord_gamma_scale, &
    ...
    coord_min = electron_four_velocity_coord_from_xgamma(coord_scale, dlog10(gamma_min))
    coord_max = electron_four_velocity_coord_from_xgamma(coord_scale, dlog10(gamma_max))
    do i = 1, num_gamma + 1
        coord_edge(i) = coord_min + (coord_max - coord_min)*dble(i - 1)/dble(num_gamma)
        x_edge(i) = electron_xgamma_from_four_velocity_coord(coord_scale, coord_edge(i))
    enddo
    do i = 1, num_gamma
        coord_mid = 0.5d0*(coord_edge(i) + coord_edge(i + 1))
        gamma_grid(i) = electron_gamma_from_four_velocity_coord(coord_scale, coord_mid)
    enddo
