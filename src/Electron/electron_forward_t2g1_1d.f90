...
subroutine fs_electron_t2g1_1d(Boundary,R_Tobs,R_Gamma,R,V_seed,n,Num_nu,Num_R,Num_gam_e,index_Y,index_syn_intger,n_threads, &
    ...
    real(8), allocatable :: dEl(:),dEL_mean(:),dEL_mean_base(:),principal(:),x(:),dF1(:),up(:), &
                            dN_x(:),dN_x_prev(:),x_edge(:),temp1(:),temp2(:),temp3(:)
    allocate (dEl(Num_gam_e),dEL_mean(Num_gam_e-1),dEL_mean_base(Num_gam_e-1),principal(Num_gam_e),x(Num_gam_e),dF1(Num_gam_e), &
              up(Num_gam_e-1),dN_x(Num_gam_e),dN_x_prev(Num_gam_e),x_edge(Num_gam_e+1), &
    ...
    deallocate(dEl,dEL_mean,dEL_mean_base,principal,x,dF1,up,dN_x,dN_x_prev,x_edge,temp1,temp2,temp3)
