...
- Electron solvers: 1D `fullhide/slc1/charint/t2g1/weno5/dg_1d` 与 2D `fullhide_2d/charint_2d` 均在当前工作树登记；`fullhide_1d` 是默认稳定基线，`dg_1d` 是 opt-in 高阶 1D 谱元路径。
...
- FS/RS shared 1D electron transport baseline 已完成：`electron_solver="dg_1d"` 同时驱动 FS 与 primary RS 电子输运；FS/RS 共用 solver id 和 shell transport primitive，RS kinetic 注入保持 \((\gamma-1)^{-p}\)，FS 注入保持 \(\gamma^{-p}\)。RS DG 状态跨 shell 持久保存，并在输出到固定 \(\log\gamma\) 网格时使用守恒 cell 投影。
- RS DG 验收基线已固定：`num_gam_e=121` 下 DG 高能尾比 fullhide 窄；`num_gam_e=241` fullhide 高能尾同步收缩，说明 121 格 fullhide 宽尾主要是低阶隐式上风数值扩散。当前不把 121 格 fullhide 高能尾作为 DG 强贴合目标；正式判断以高分辨率 fullhide、解析特征线或守恒谱形诊断为准。记录图见 `output/asgard_doc/reverse_dg_fullhide_electron_spectrum/`。
...
`tests/dg_1d_smoke.py` 当前保留为严格诊断：FS density-jump 场景仍触发 3 个 grid-scale sawtooth turns，见 `TODO.md`；它不是本基线的通过门槛。
