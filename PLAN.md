...
- Electron solvers: 1D `fullhide/slc1/charint/t2g1/weno5/dg_1d` 与 2D `fullhide_2d/charint_2d` 均在当前工作树登记；`fullhide_1d` 是默认稳定基线，`dg_1d` 是 opt-in 高阶 1D 谱元路径。`charint_2d` 的 downstream chi 能量列使用公共无源特征线冷却 primitive，shock-front 注入和 eta 输运仍保持 2D transport 方程。
