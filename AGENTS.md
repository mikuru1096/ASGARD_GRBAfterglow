...
- **Pair cascade**: `pair_cascade_iterations > 1` now uses a shell-sequence time-dependent γγ pair/synch cascade path; the legacy single-shell iterative kernel remains for low-level diagnostics. IC-mediated electromagnetic cascade is not implemented until photon/e± source-sink equations and an energy-budget benchmark are defined.
