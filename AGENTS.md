...
- **Magnetized RS overlay refresh**: `scripts/benchmarks/magnetized_rs_sigma_benchmark.py` 当前 ISM/wind 参数固定为 `E_iso=1e54`, `Gamma0=50`, `duration=500 s`, `rvs_eps_e=0.3`, `rvs_eps_B=0.1`；wind 为纯 `Astar=0.05` 且 `n_ism=1e-15` floor。ASGARD/Vegas overlay 使用 `scripts/benchmarks/magnetized_rs_vegas_lc_compare.py --medium ism|wind`；VegasAfterglow 2.0.5 需先应用 `doc/patches/vegasafterglow-2.0.5-rvs-ism-compression.patch`，否则纯 ISM magnetized RS synchrotron 会全 0。
- **Triple density-jump RS+FS benchmark**: `scripts/benchmarks/triple_density_jump_rs_fs_tophat.py --mode formal` 使用 ASGARD/Vegas baseline top-hat 参数并固定 `Gamma0=100`、RS upstream `sigma=0.1`，`jump_r_cm=(1e14,1e15,1e16) cm`、`jump_factor=100`、半径空间 normal Gaussian `sigma_R/R_jump=0.1`，输出 `output/asgard_doc/reverse_density_jump_tests/triple_density_jump_rs_fs_tophat.png/.svg/.pdf/.tiff` 的射电/光学/X 射线 FS+RS 光变。
...
- **Time-dependent BH/joint benchmark**: `scripts/benchmarks/time_dependent_bh_photon_benchmark.py --mode formal` 覆盖 weak-feedback、BH-active、strong-wind-BH 的 separated/joint 对比；输出到 `output/asgard_doc/time_dependent_bh_photon_benchmark/`，属于可复现 artifact，清理提交时默认不纳入版本控制。
